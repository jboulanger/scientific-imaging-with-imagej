...
\beamertemplateshadingbackground{white}{teal!8!gray!10}
% \beamertemplateshadingbackground{orange!2!white}{brown!10!white}
% \beamertemplateshadingbackground{white}{blue!10!gray!15}
...
\definecolor{exbg}{RGB}{240,244,248} % Light blue background
\definecolor{exfg}{RGB}{31,58,82}   % Dark blue text
% \setbeamercolor{exercisecolor}{fg=exfg, bg=exbg}
\setbeamercolor{exercisecolor}{bg=teal!10!gray!20, fg=blue!10!black}
...
\begin{frame}{Exercise: Installation \& Update sites}
  \noindent\begin{beamercolorbox}[rounded=true, shadow=true,sep=-1em]{exercisecolor}%
    To perform the exercises we will first install Fiji and the necessary packages.
    \begin{itemize}
...
\begin{frame}{Exercise: Installation \& Update sites}
  ...
  \begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth, sep=-1em]{exercisecolor}%
    \begin{enumerate}
...
\begin{frame} \frametitle{Microscope vendor image formats}
  ...
  Fiji uses \textbf{\href{https://imagej.net/formats/bio-formats}{bioformats}} to load these files (installed by default).
  ...
  \begin{itemize} \setlength\itemsep{1em}
    ...
    \item Acquisition software collect additional information that are
          ...
          \begin{itemize}
            ...
            \item \dots
          \end{itemize}
    \item The Open Microscopy Environment (OME) defines a \textbf{\href{http://www.openmicroscopy.org/Schemas/OME/2016-06}{specification}}
...
\begin{frame}{Exercise: Scale bar}
...
\begin{frame}{Exercise: Brightness \& Contrast}
  \begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth, sep=-1em]{exercisecolor}%
    \begin{enumerate}
...
\begin{frame} \frametitle{Histogram}
  ...
  \begin{columns}
    ...
    \begin{column}{0.5\textwidth}
      \begin{itemize}\setlength\itemsep{1em}
        \item Frequency of each intensity values.
...
\begin{frame}{Exercise: Look up table (LUTs)}
  \begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth, sep=-1em]{exercisecolor}%
    \begin{enumerate}
...
\begin{frame}{Multichannel images}

  \begin{itemize} \setlength\itemsep{1em}
    \item \textbf{Channel:} one detector measuring one specific signal at a time.
    \item Multichannel images can be easily acquired by microcopes
          (multiple wavelength/fluorophores, brightfield/DIC, \dots)
    \item There are two types of colour images:
          \begin{itemize}
            \item RGB images store colour information into a single ($8 \times
                    3$) 24 bit image.
    ...
    \item Create a multichannel image using \menu{Image>Color>Merge
            Channels\dots}.
...
\begin{frame}{Exercise: Multichannel image}
  \begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth, sep=-1em]{exercisecolor}%
    \begin{enumerate}
...
\begin{frame} \frametitle{Multidimensional images: stack \& hyper stacks}
  ...
  \begin{itemize} \setlength\itemsep{1em}
    ...
    \item Hyper stacks can have up to 5 dimensions xyzct with no limits
          on the number of channels.
    \item Virtual stacks enable to load only the images planes which are
          visualized.
  \end{itemize}
  \vspace{1em}
  To quickly visualize multidimensional data, use :
  \vspace{0.5em}
  \begin{itemize}
    ...
    \item Re-slice a stack with \menu{Image>Stack>Reslice\dots} or press \keys{/}
    \item To synchronize two stacks use \menu{Analyze>Tools>Synchronize Windows}
    \item Visualize 3D data using \menu{Plugins>3D viewer}, ClearVolume, or 3D script
  \end{itemize}
\end{frame}
...
\begin{frame}{Filtering, noise \& edges}
  \begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth, sep=-1em]{exercisecolor}%
    \begin{enumerate}
...
\begin{frame}{Binary image processing}
  \begin{itemize} %\setlength\itemsep{1em}
...
\begin{frame}{From thresholds to regions of interests}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
...
\begin{frame}{Thresholding \& Selection}
  \begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth, sep=-1em]{exercisecolor}%
    \begin{enumerate}
...
\begin{frame}{Thresholding \& Particle analysis}
  \begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth, sep=-1em]{exercisecolor}%
    \begin{enumerate}
...
\begin{frame}{Seeded watershed}
  \begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth,sep=-0.5em]{exercisecolor}%
    \begin{enumerate}
...
\begin{frame} \frametitle{Colocalisation: Manders' overlap coefficient}
  \begin{columns}
    \begin{column}{0.7\textwidth}
      ...
      \begin{block}{Pro tip}
        \begin{itemize}
          \item Object segmentation and background correction will improve colocalisation analysis.
          \item Co-localization does not prove interaction.
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.3\textwidth}
...
\begin{frame}{Computing manually the Pearson correlation coefficient}
  \begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth,sep=-0.5em]{exercisecolor}%
    \begin{enumerate}
...
\begin{frame}{Pixel classification with Labkit}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
      ...
      \begin{block}{Pro tip}
        \begin{itemize}
          \item Annotate the training data with thin lines.
          \item Add classes for different type of artifacts.
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.5\textwidth}
...
\begin{frame}{Nuclei segmentation with Stardist}
  \begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth, sep=-0.5em]{exercisecolor}%
    \begin{enumerate}
      \item Open the "HeLa Cells (48-bit RGB)" example
...
\begin{frame} \frametitle{Image integrity}
  ...
  information (check our wiki \url{http://light-microscopy-wiki/} for details on equipment):
  \begin{itemize}\small
  ...
  \footfullcitenomark{monterollopisBestPracticesTools2021}
...
\begin{frame} \frametitle{Data repository}
  ...
  \begin{itemize}
    \item BioImage Archive \url{https://www.ebi.ac.uk/bioimage-archive/} (viewer)
    \item Image Data Repository \url{https://idr.openmicroscopy.org} (viewer)
    \item Zenodo \url{https://zenodo.org} (CERN, doi)
    \item Figshare \url{https://figshare.com} (20GB, doi)
    \item DRYAD \url{https://datadryad.org} ()
  \end{itemize}

  More on data management best practices:
  \begin{itemize}
