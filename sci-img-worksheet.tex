...
\begin{exercice} \textbf{Installation \& Update sites}
  To perform the exercices we will first install Fiji and the necessary packages.
  \begin{itemize}
...
\begin{exercice} \textbf{3D measurements}
  \begin{enumerate}
    \item Open the "Organ of corti" sample image.
    \item Duplicate twice the first channel
    \item On one of the duplicated image, apply a Median filter in 3D and threshold the image
    \item Compute the connected components (\menu{Plugins>MorpholibJ>Binary Images>Distance Transform Watershed 3D})
    \item Measure intensities in the original image using \menu{Plugins>MorpholibJ>Analyze>Intensity Measurements 2D/3D}
  \end{enumerate}
\end{exercice}
